...
function login(){
  ...
  let user = users.find(
    u => u.username === account && u.password === pass
  );
...
function bet(side, amount){
  ...
  if(side === "正面"){
    ...
    userBet.front = Number(
      (userBet.front + amount).toFixed(2)
    );
...
function settle(resultSide){
  ...
  /* 连胜 */
  if(streakSide === resultSide){

    streakCount++;

  }else{

    streakSide = resultSide;
  ...
  if(winAmount > 0){
    ...
    let payout = Number(
      (winAmount * 1.9).toFixed(2)
    );

    let profit = Number(
      (payout - winAmount - loseAmount).toFixed(2)
    );
...
function renderWinRoad(road){
  ...
  let map =
    document.getElementById("winRoadMap");
...
function explodeCoins(){
  ...
  let box =
    document.getElementById("coinExplosion");
