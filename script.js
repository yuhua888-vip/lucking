...
function saveUser(){
  ...
  localStorage.setItem("user_" + currentUser, JSON.stringify(old));
...
function updateFakeMarket(){
  ...
  document.getElementById("fakeFrontTotal").innerText = fakeFrontTotal.toFixed(0);
  document.getElementById("fakeBackTotal").innerText = fakeBackTotal.toFixed(0);

  let total = fakeFrontTotal + fakeBackTotal;
  let frontPercent = total ? (fakeFrontTotal / total) * 100 : 50;
  let backPercent = 100 - frontPercent;

  document.getElementById("frontHeat").style.width = frontPercent + "%";
  document.getElementById("backHeat").style.width = backPercent + "%";

  document.getElementById("hotText").innerText =
    frontPercent > backPercent ? "🔥 热门：正面" : "🔥 热门：反面";
}
...
function addRoad(side){
  ...
  dot.className = "road-dot " + (side === "正面" ? "front" : "back");
  dot.innerText = side === "正面" ? "正" : "反";
...
function addWinRoad(text){
  ...
  let dot = document.createElement("div");
  dot.className = "road-dot " + (text === "赢" ? "win" : "lose");
  dot.innerText = text;
...
setInterval(() => {
  ...
  countdownNum--;
  countdown.innerText = "倒计时：" + countdownNum;
...
function createChipButtons(){
  if(document.getElementById("chipButtons")) return;

  let control = document.querySelector(".chip-bet-control");
  ...
  arr.forEach(item => {
    ...
    btn.style.background = "linear-gradient(145deg,#facc15,#f59e0b)";
    btn.style.fontWeight = "900";

    btn.onclick = () => {
      document.getElementById("baccaratBetAmount").value = item;
    };
...
function selectBaccaratBet(side){
  ...
  document.getElementById("selectedBetSide").innerText = "当前选择：" + side;
...
function placeBaccaratBet(){
  ...
  let amount = Number(document.getElementById("baccaratBetAmount").value);
  ...
  document.getElementById("bankerBet").innerText = baccaratBetData["庄"].toFixed(2);
  document.getElementById("playerBet").innerText = baccaratBetData["闲"].toFixed(2);
  document.getElementById("tieBet").innerText = baccaratBetData["和"].toFixed(2);
...
function clearBaccaratBets(){
  chips += baccaratBetData["庄"] + baccaratBetData["闲"] + baccaratBetData["和"];
...
function startBaccaratRound(){
  ...
  document.getElementById("baccaratResult").innerText =
    "本局结果：" + resultSide + " ｜ 获得：" + reward.toFixed(2);
...
function updateBaccaratRoad(){
  let road = document.getElementById("baccaratRoad");
  road.innerHTML = "";
...
function createGoldParticle(){
  if(!gameBox || gameBox.classList.contains("hidden")) return;
...
setInterval(createGoldParticle, 900);
